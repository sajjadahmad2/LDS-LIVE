...
class ContactServices
{
    public function handleContact(array $data, $agent=null,$campaign)
    {
       ...
       // dd($agent);
        ...
        if ($contact) {
            ...
            //$this->forwardContactData($contact, $agent);
        } else {
            $contact = new Contact();
            foreach ($contactData as $key => $value) {
                $contact->$key = $value;
    ...
    private function forwardContactData($contact, $agent)
    {
        if (!empty($agent->destination_webhook)) {
            ...
            $sendContactToGHLServices = new sendContactToGHLServices;
            $sendContactToGHLServices->sendContactToGHL($contact->toArray(), $agent);
        }
    }
