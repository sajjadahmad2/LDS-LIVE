...
class ProccessContactJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $proccessContact = ProccessContact::where('email', $this->data['email'])->first();
        ...
        $contactData = [
            ...
            'user_id' => $this->agent ? $this->agent->user_id : null,
        ...
        if ($proccessContact) {
            ...
            $proccessContact->save();
             \Log::info("Updated  old Contact  in Process Contact Table with contact ID: {$contact_id} With status NOT SENT");
             ContactJob::dispatch($proccessContact, $agent,$this->campaign);
        } else {
            ...
            $proccessContact->save();
            \Log::info("Created new Contact   in Process Contact Table with contact ID: {$contact_id} With status NOT SENT");
             ContactJob::dispatch($proccessContact, $agent,$this->campaign);
